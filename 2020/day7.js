//AOC2020 D7
const parseRule = s => {
    var m = s.match(/(\w+ \w+) bags? contain (.*)/);
    return {
        id: m[1],
        bags: m[2] === 'no other bags.'
            ? [] 
            : m[2].replace(/\.$/, '').split(', ').map(b => {
                let _m = b.match(/\s*(\d+)\s+(\w+ \w+) bags?/);
                return { qty: parseInt(_m[1], 10), id: _m[2]};
            })
    }
}
...
function findBags(bags) {
    ...
    var _rules = rules.filter(r => r.bags.map(r => r.id).some(id => bags.includes(id)));
    return [..._rules, ...findBags(_rules.map(r => r.id))];
}

console.log(new Set(findBags(['shiny gold'])).size);

function countBags(id) {
    var rule = rules.find(r => r.id === id);
    return rule.bags.reduce((a, c) => a + c.qty + c.qty * countBags(c.id), 0);
}
...
function data() {
    return `dark orange bags contain 3 dark chartreuse bags.
