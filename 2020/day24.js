...
function ints(s) { return s.split(',').map(Number); }

function move(pt, action) {
    let [x,y,z] = Array.isArray(pt) ? pt : ints(pt);
    if(action === 'e') { x += 1; y -= 1; }
    if(action === 'w') { x -= 1; y += 1; }
    if(action === 'se') { x += 1; z -= 1; }
    if(action === 'nw') { x -= 1; z += 1; }
    if(action === 'sw') { z -= 1; y += 1; }
    if(action === 'ne') { z += 1; y -= 1; }
    return [x,y,z];
}
...
neeswseeseeeeeseeeeeeeseee`.trim();
}
