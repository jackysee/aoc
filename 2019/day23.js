...
function createNic(addr) {
    let prg = createProgram(data());
    let queue = [];
    queue.push(addr, -1);
...
// console.log(createNic(5).run([-1]));
// console.log(createNic(0).run([]));
...
function run(part1) {
    ...
    let lastPacketAddr;
    let nat = undefined;
    let natHistory = new Set();
    function send(packets) {
        while (packets.length) {
            let [addr, x, y, ...rest] = packets;
            if (addr === 255) {
                nat = [x, y];
                if (part1) return y;
            }
            if (nics[addr]) {
                nics[addr].receive(x, y);
            }
            packets = rest;
            lastPacketAddr = addr;
        }
    ...
    function queues() {
        return Object.values(nics).reduce(
            (acc, nic) => acc + nic.queue().length,
            0
        );
    }
