...
function addPoints(points, num, mapper){
    return [...points, ...[...Array(num)].map((_,i) => mapper(i))];
}

function walk(steps, [x, y] = [0, 0], points = []) {
    if(!steps.length) {
        return points;
    }
    let [s, ...rest] = steps;
    if(s.op === 'R') {
        points = addPoints(points, s.num, i => `${x+i+1},${y}`);
        x += s.num;
    }
    if(s.op === 'L') {
        points = addPoints(points, s.num, i => `${x-i-1},${y}`);
    ...
    if(s.op === 'U') {
        points = addPoints(points, s.num, i => `${x},${y-i-1}`);
        y -= s.num;
    }
    if(s.op === 'D') {
        points = addPoints(points, s.num, i => `${x},${y+i+1}`);
    ...
    return walk(rest, [x,y], points);
}


function populate(wire) {
    let steps = wire.split(',').map(s => {
        const m = s.match(/(\w)(\d+)/);
        return { op: m[1], num: parseInt(m[2], 10)}
    });
    return walk(steps);
}
let points1 = populate(wire1);
let points2 = populate(wire2);
let set1 = new Set(points1);
let set2 = new Set(points2);
let cross = [...set1].filter(p => set2.has(p));

console.log(Math.min(...cross.map(p => {
    let [x,y] = p.split(',').map(s => parseInt(s,10));
    return Math.abs(x) + Math.abs(y);
})));

console.log(Math.min(...cross.map(p => {
    var i1 = points1.indexOf(p);
    var i2 = points2.indexOf(p);
    return i1 + i2 + 2;
})))
