...
function bfs(start, dest, map) {
    let visited = new Set();
    let queue = [ { p:start.pos, val:start.val, dist:0, doors:[], keys:[] } ];
    while(queue.length !== 0) {
        ...
        let points = [ [x-1,y], [x+1,y], [x,y-1], [x,y+1] ]
            ...
            .map(p => {
                return {
                    p, dist: pos.dist + 1, 
                    doors: [...pos.doors, /[A-Z]/.test(map[p]) ? map[p]: false].filter(Boolean), 
                    keys: [...pos.keys, /[a-z]/.test(map[p]) && map[p] !== dest.val ? map[p]: false].filter(Boolean), 
                }
            });
...
function dist(G, from = '@0', keys = new Set()) {
    ...
    let nodes = Object.entries(G[from])
        .filter(([n, {doors}]) => {
            return !keys.has(n) && 
