...
class Player {
    ...
    duplicate() {
        ...
        for (var i = 0; i < this.activeSpells.length; i++) {
            newPlayer.activeSpells.push(
                Object.assign({}, this.activeSpells[i])
            );
        }
...
function playAllGames(me, boss, partTwo, depth) {
    ...
    for (var i = 0; i < me.spells.length; i++) {
        ...
        for (var j = 0; j < me.activeSpells.length; j++) {
            if (
                me.activeSpells[j].duration > 1 &&
                i === me.activeSpells[j].idx
            ) {
                spellMatch = true;
            }
        }
        ...
        if (newBoss.hp <= 0) {
            if (cheapestSpent < newMe.spent) cheapestMe = newMe.duplicate();
